...
let Acontent = fastas.map( { $0.A }).sum()
let Ccontent = fastas.map( { $0.C }).sum()
let Gcontent = fastas.map( { $0.G }).sum()
let Tcontent = fastas.map( { $0.T }).sum()
let Ncontent = fastas.map( { $0.N }).sum()

//let totalNucleotideContent = NcountScaffsLess1K + NcountScaffsGtr1K + NcountScaffsGtr10K + NcountScaffsGtr100K + NcountScaffsGtr1M + NcountScaffsGtr10M
//print(totalNucleotideContent)
